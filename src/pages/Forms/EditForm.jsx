...
const EditForm = ({ languageText, language, api }) => {
    ...
    // const removeSelectInput = (id) => {
    //     setSelectInputs((prev) => prev.filter((input) => input._id !== id));
    // };
    ...
    // const addOption = (id) => {
    //     const updatedInputs = selectInputs.map((item) =>
    //         item._id === id ? { ...item, options: [...item.options, ""] } : item
    //     );
    //     setSelectInputs(updatedInputs);
    // };
    ...
    // const toggleMultiSelect = (id) => {
    //     const updatedInputs = selectInputs.map((item) =>
    //         item._id === id ? { ...item, isMultiSelect: !item.isMultiSelect } : item
    //     );
    //     setSelectInputs(updatedInputs);
    // };
    ...
    useEffect(() => {
        if (formData && !formLoading && !formError) {
            ...
            setArabicEventName(formData.arabicEventName);
            setEventImg(formData.eventImg);
            setEventDescription(formData.eventDescription);
            setGroupLink(formData.groupLink);
            ...
            const transformedSelectInputs = formData.selectInputs.map((input, index) => ({
                ...input,
                id: `${Date.now()}-${index}` // Ensure unique ID
            }));
            setSelectInputs(transformedSelectInputs);
            setCustomInputs(transformedCustomInputs);
            // setSelectInputs(formData.selectInputs);
    ...
    const onSubmit = async (e) => {
        ...
        const updatedFormData = {
            ...
            limit: formLimit,
            selectInputs: customSelectForBackend
        };

        try {
            await handleSubmit(`${api}/api/forms/${id}`, "PATCH", updatedFormData, "forms", languageText.FormSuccessUpdateMessage);
            setError("");
            navigate(-1)
        } catch (error) {
                            ...
                            <AnimatePresence>
                                ...
                                >
                                    ...
                                    <h1
                                        className={`formTitle`}>
                                        {(() => {
                                            const words = languageText.EditForm.split(" ");
